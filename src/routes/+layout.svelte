				...
				if (done) {
					if ($isLastActiveTab) {
						if ($settings?.notificationEnabled ?? false) {
							new Notification(`${title} | Sawl GPT`, {
								body: content,
	...
	const channelEventHandler = async (event) => {
		...
		if ((!channel || isFocused) && event?.user?.id !== $user?.id) {
			...
			if (type === 'message') {
				if ($isLastActiveTab) {
					if ($settings?.notificationEnabled ?? false) {
						new Notification(`${data?.user?.name} (#${event?.channel?.name}) | SAWL GPT`, {
							body: data?.content,
...
<div class="fixed bottom-4 right-4 text-sm text-gray-500">
	문의: 경기과고 신병철 sinbc2004@naver.com <br /> 오픈소스로 제작, 민감한 정보 입력금지, 수원외고 교사만 이용가능
</div>
